...
- [ ] T016 [US1] Author `.github/workflows/gate.yml` reusable workflow — inputs `adapter` + `license_side`, runs every required category from `gate-policy.yml`, rejects `policy_overrides` loudly, emits one check per category (FR-001, FR-003, SC-003, SC-005; inherits T011 baseline)
...
- [ ] T019 [P] [US1] Quality category license-side declaration check (`agpl-core` | `apache-edge`) (Constitution III)
